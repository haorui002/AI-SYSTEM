...
class StyleLossLayer(object):
    ...
    def forward(self, input_layer, style_layer):
        ...
        style_diff = np.sum(np.square(self.gram_style - self.gram_input)) / (self.div*4*input_layer.shape[0])
        loss = style_diff
        return loss
    def backward(self, input_layer, style_layer):
        ...
        for idxn in range(input_layer.shape[0]):
            # TODO： 计算风格损失的反向传播
            bottom_diff[idxn, :, :] = np.sum(np.matmul(np.transpose(style_layer,[2,3,0,1]), ((self.gram_input[idxn, :, :] - self.gram_style[idxn, :, :])))) / (self.div*input_layer.shape[0])
