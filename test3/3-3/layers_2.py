            ...
            for idxc in range(top_diff.shape[1]):
                for idxh in range(top_diff.shape[2]):
                    for idxw in range(top_diff.shape[3]):
                        ...
                        self.d_weight[:, :, :, idxc] += np.multiply(self.input_pad[idxn, :, idxh*self.stride:idxh*self.stride+self.kernel_size, idxw*self.stride:idxw*self.stride+self.kernel_size], top_diff[idxn, idxc, idxh, idxw])
...
class MaxPoolingLayer(object):
    ...
    def backward_speedup(self, top_diff):
        ...
        bottom_diff = self.backward_raw_book(top_diff)
        return bottom_diff
