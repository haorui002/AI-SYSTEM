...
class FullyConnectedLayer(object):
    ...
    def forward(self, input): # 前向传播计算  一行，m列
        start_time = time.time()
        self.input=input
        # TODO：全连接层的前向传播，计算输出结果
        self.output=self.input @ self.weight + self.bias  #一行，n列
        return self.output

    def backward(self, top_diff):   # 反向传播的计算  p（1）行，n列
        # TODO：全连接层的反向传播，计算参数梯度和本层损失
        self.d_weight=self.input.T @ top_diff               #m行，n列
        self.d_bias=np.ones([1, self.input.shape[0]]) @ top_diff    #一行，n列
        bottom_diff=top_diff @ self.weight.T      #一行m列
        return bottom_diff
